...
#define REPBUFSIZE 32  /* 応答メッセージ受信バッファのサイズ */
#define MAXWORD 140 /* 送信する文字のバイト数上限 */
#define OWARI 1

void DieWithError(char *errorMessage);  /* エラー処理関数 */
int mojiInput(char echoString[]); /* 文字入力 */
...
int main(int argc, char *argv[])
{
	...
	char echoString[MAXWORD];        /* エコーサーバに送信する文字列 */
	char replyBuffer[REPBUFSIZE];     /* 応答メッセージ受信用バッファ */
	char echoBuffer[MAXWORD];     /* エコー文字列用バッファ */
	unsigned int echoStringLen = 32;      /* エコーする文字列のサイズ */
	...
	/* 接続okならサーバーから Connect OK! の文字が返ってくる。 */
	if ((bytesRcvd = recv(sock, replyBuffer, REPBUFSIZE - 1, 0)) <= 0)
	...
	for(;;) {
		...
		while (totalBytesRcvd < echoStringLen) {
			/* バッファサイズに達するまで(NULL文字用の1バイトを除く)
			   ...
			   0 -- ソケットが呼び出されたときの振る舞い（デフォルト）
			*/
			if ((bytesRcvd = recv(sock, echoBuffer, MAXWORD - 1, 0)) <= 0)
				DieWithError("recv() failed or connection closed prematurely");
			/* 総バイト数の集計 */
